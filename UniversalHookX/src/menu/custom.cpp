...
bool c_custom::expand_button( const char* label, bool selected, float rounding, ImDrawFlags flags ) {
    ...
    bool hovered, held;
    bool pressed = ButtonBehavior( rect, id, NULL, &held, NULL );
  ...
  //  draw->AddImage(logo, ImVec2(rect.Min.x + 55 / 2 - icon_size.x / 2, rect.GetCenter().y - icon_size.y / 2), ImVec2(rect.Min.x + 55 / 2 + icon_size.x / 2, rect.GetCenter().y - icon_size.y / 2),  ImVec2(0, 0), ImVec2(1, 1), ImColor(1.f, 1.f, 1.f, 1.f));
//draw->AddText( GetIO( ).Fonts->Fonts[1], GetIO( ).Fonts->Fonts[1]->FontSize - 2, ImVec2( rect.Min.x + 55 / 2 - icon_size.x / 2, rect.GetCenter( ).y - icon_size.y / 2 ), GetColorU32( ImGuiCol_Text, ( value->second > 0.3f ? value->second : 0.3f ) ), "A" );
    if (shouldexp)
...
bool c_custom::tab( const char* icon, const char* label, bool selected ) {
    ...
    ImVec4 accent_color = ImColor(250, 132, 86);

  //  window->DrawList->AddShadowCircle({ rect.GetCenter().x, rect.GetCenter().y }, 3.0f, ImColor(1.f, 1.0f, 0.0f, 0.18f), 150.0f, { 0, 0 });
   // draw->AddShadowCircle(ImVec2(rect.GetCenter().x, rect.GetCenter().y ), 5.f, GetColorU32(accent_color), 50.f, ImVec2(0, 0), 0, 12);

    draw->AddText( GetIO( ).Fonts->Fonts[1], GetIO( ).Fonts->Fonts[1]->FontSize, ImVec2( rect.Min.x + 55 / 2 - icon_size.x / 2, rect.GetCenter( ).y - icon_size.y / 2 ), GetColorU32( ImGuiCol_Text, ( value->second > 0.3f ? value->second : 0.3f ) ), icon );
    if(shouldexp)
    draw->AddText( ImVec2( rect.Min.x + 50, rect.GetCenter( ).y - label_size.y / 2 + 2 ), GetColorU32( ImGuiCol_Text, ( value->second > 0.3f ? value->second : 0.3f ) ), label );
...
void c_custom::tab_area( const char* str_id, ImVec2 size, std::function < void ( ) > content ) {
    ...
    value->second.width = ImLerp( value->second.width, ( value->second.should_expand && value->second.hovered ) ? size.x + 50 : size.x , 0.15f );
    ...
    ImVec4 newChildBgColor = ImVec4(17 / 255.0f, 17 / 255.0f, 18 / 255.0f, 1.0f);

    PushStyleColor(ImGuiCol_ChildBg , newChildBgColor);
    ...
    GetWindowDrawList( )->AddRectFilled( GetCurrentWindow( )->Pos, GetCurrentWindow( )->Pos + GetCurrentWindow( )->Size, ImColor(17, 17, 18), GetStyle( ).WindowRounding, ImDrawFlags_RoundCornersTopLeft );
    ...
    shouldexp = value->second.should_expand;
    content( );
...
void c_custom::begin_child( const char* name, ImVec2 size ) {
    ...
    GetWindowDrawList( )->AddRectFilled( pos, pos + size, to_vec4(17, 17, 18, custom.m_anim ), 4 );
